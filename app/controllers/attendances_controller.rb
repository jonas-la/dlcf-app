class AttendancesController < ApplicationController
  ...
  def create2
    ...
    if existing_attendance
      ...
      respond_to do |format|
        format.html { redirect_to event_path(attendance_params[:event_id]), notice: "Attendance record already exists." }
        format.json { render json: { error: "Attendance record already exists" }, status: :unprocessable_entity }
      end
    else
      @attendance = Attendance.new(
        member_id: attendance_params[:member_id], 
      ...
      respond_to do |format|
        if @attendance.save
          format.html { redirect_to event_path(attendance_params[:event_id]), notice: "#{Member.find(attendance_params[:member_id]).first_name} successfully signed in." }
          format.json { render :show, status: :created, location: @attendance }
        else
          format.html { redirect_to event_path(attendance_params[:event_id]), notice: "Error signing in. Not valid" }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to event_path(event_id), notice: "Attendance was successfully destroyed." }
